...
bool MediaPlayerPrivateGStreamerBase::handleSyncMessage(GstMessage* message)
{
    ...
    if (gst_is_video_overlay_prepare_window_handle_message (message)) {
        ...
        //set a size so 0,0,1,1 (x,y,w,h) so we don't error on <waylandsink> error: Window has no size set
        //the holePunchClient will update the GstOverlay render-rect with the correct size
        gst_video_overlay_set_render_rectangle(m_gstVideoOverlay.get(), 0, 0, 1, 1);
        return true;
    }
